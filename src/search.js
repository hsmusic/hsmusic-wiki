...
async function populateSearchIndexes(indexes, wikiData) {
  ...
  readCollectionIntoIndex(
    ...
    track => ({
      ...
      artists: [
        track.artistContribs.map(contrib => contrib.who.name),
        ...track.artistContribs.map(contrib => contrib.who.aliasNames)
      ].flat(),
      additionalNames: track.additionalNames.map(who => who.name),
    })
  );
  ...
  readCollectionIntoIndex(
    ...
    flash => ({
      ...
      contributors: [
        flash.contributorContribs.map(contrib => contrib.who.name),
        ...flash.contributorContribs.map(contrib => contrib.who.aliasNames)
      ].flat()
    })
  );
}
